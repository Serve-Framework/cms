...
class Comment extends Helper
{
    ...
    public function comments_open(int $post_id = null): bool
    {
        ...
        {
            return $this->query->post->comments_enabled == true;
        }
    ...
    {
        return !empty($this->query->get_comments($post_id));
    }
    ...
    {
        return count($this->query->get_comments($post_id));
    }
     ...
     * @return \cms\wrappers\Comment|null
     */
    ...
    public function get_comments(int $post_id = null): array
    {
        ...
        if (!empty($this->query->post))
        {
            return $this->query->post->comments;
        }
    ...
    public function display_comments(array $args = null, int $post_id = null): string
    {
        ...
        if ($this->query->comments_number($post_id) === 0)
        {
            return '';
        }
        ...
        $post  = !$post_id ? $this->query->post : $this->query->helper('cache')->getPostByID($post_id);

        // Fallback incase nothing is present
        if (!$post || empty($post)) return '';

        // Save the article permalink locally
        $permalink = $this->query->the_permalink($post->id);
        ...
        $formTemplate = $this->query->theme_directory() . DIRECTORY_SEPARATOR . 'comments.php';

        if (file_exists($formTemplate))
        {
            return $this->query->include_template('comments', ['comments' => $comments]);
        }
    ...
    {
        // Load from template if it exists
        $formTemplate = $this->query->theme_directory() . DIRECTORY_SEPARATOR . 'commentform.php';
        if (file_exists($formTemplate)) return $this->query->include_template('commentform');
        ...
        // Save the article permalink locally
        $permalink   = $this->query->the_permalink($postID);
        ...
        if (!$this->query->comments_open($post_id))
        {
    ...
    private function commentToString(array $comments, array $options, string $permalink, bool $isChild = false): string
    {
        ...
        foreach ($comments as $comment)
        {
            ...
            // Replace avatar src
            $patterns[]     = '/\(:avatar_src\)/';
            $replacements[] = $this->query->get_gravatar($comment->email, $options['avatar_size'], true);
