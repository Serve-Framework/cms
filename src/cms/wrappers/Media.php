...
class Media extends Wrapper
{
    ...
    public function save(): bool
	{
        ...
        if ($mediaExists)
        {
            $saved = $this->SQL->UPDATE('media_uploads')->SET($this->data)->WHERE('id', '=', $mediaExists['id'])->EXEC();
        }
        else
        {
            $saved = $this->SQL->INSERT_INTO('media_uploads')->VALUES($this->data)->EXEC();
    ...
    public function delete(): bool
    {
        if (isset($this->data['id']))
        {
            ...
            $this->SQL->UPDATE('posts')->SET(['thumbnail_id' => null])->WHERE('thumbnail_id', '=', $this->data['id'])->EXEC();

            return $this->SQL->DELETE_FROM('media_uploads')->WHERE('id', '=', $this->data['id'])->EXEC() ? true : false;
        }
