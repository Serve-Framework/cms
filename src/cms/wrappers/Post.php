...
class Post extends Wrapper
{
	...
	public function save(): bool
	{
		...
		if ($newPost)
		{
			...
			$this->SQL->INSERT_INTO('posts')->VALUES($insertRow)->EXEC();
		...
		{
			$this->SQL->UPDATE('posts')->SET($insertRow)->WHERE('id', '=', $row['id'])->EXEC();

			$this->SQL->DELETE_FROM('tags_to_posts')->WHERE('post_id', '=', $row['id'])->EXEC();

			$this->SQL->DELETE_FROM('categories_to_posts')->WHERE('post_id', '=', $row['id'])->EXEC();

			$this->SQL->DELETE_FROM('content_to_posts')->WHERE('post_id', '=', $row['id'])->EXEC();

			$this->SQL->DELETE_FROM('post_meta')->WHERE('post_id', '=', $row['id'])->EXEC();
		}

		// Join the tags
		foreach ($row['tags'] as $tag)
		{
			$this->SQL->INSERT_INTO('tags_to_posts')->VALUES(['post_id' => $row['id'], 'tag_id' => $tag->id])->EXEC();
		}

		// Join the categories
		foreach ($row['categories'] as $cat)
		{
			$this->SQL->INSERT_INTO('categories_to_posts')->VALUES(['post_id' => $row['id'], 'category_id' => $cat->id])->EXEC();
		}

		// Join the content
		$this->SQL->INSERT_INTO('content_to_posts')->VALUES(['post_id' => $row['id'], 'content' => $row['content']])->EXEC();

		// Join the post meta
		if (!empty($postMeta))
		{
			$this->SQL->INSERT_INTO('post_meta')->VALUES(['post_id' => $row['id'], 'content' => Str::mysqlEncode(serialize($postMeta))])->EXEC();
		}
	...
	public function delete(): bool
	{
		...
		{
			$this->SQL->DELETE_FROM('comments')->WHERE('post_id', '=', $this->data['id'])->EXEC();

			$this->SQL->DELETE_FROM('tags_to_posts')->WHERE('post_id', '=', $this->data['id'])->EXEC();

			$this->SQL->DELETE_FROM('categories_to_posts')->WHERE('post_id', '=', $this->data['id'])->EXEC();

			$this->SQL->DELETE_FROM('content_to_posts')->WHERE('post_id', '=', $this->data['id'])->EXEC();

			$this->SQL->DELETE_FROM('posts')->WHERE('id', '=', $this->data['id'])->EXEC();

			$this->SQL->DELETE_FROM('post_meta')->WHERE('post_id', '=', $this->data['id'])->EXEC();
     ...
     *
     * @param  \cms\wrappers\Category|null $category Category wrapper
     * @return string
     */
